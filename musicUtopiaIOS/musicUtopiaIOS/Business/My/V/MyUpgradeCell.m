...
-(void)layoutSubviews {
    ...
    _instrumentIcon.frame = CGRectMake(CONTENT_MARGIN_LEFT,[_cellBox height]/2 - 30/2 - 2,30, 30);
    ...
    _goLevelHint.frame = CGRectMake([_rightIcon left] - 55 , [_cellBox height]/2 - ATTR_FONT_SIZE/2, 60, ATTR_FONT_SIZE);
...
-(void)setDictData:(NSDictionary *)dictData {
    ...
    //获取类别图标
    NSDictionary * categoryData = [BusinessEnum getCategoryId:[dictData[@"ul_cid"] integerValue]];
    _instrumentIcon.L_ImageName(categoryData[@"icon"]);
    
    _instrumentText.L_Text(categoryData[@"c_name"]);
    
    _nowLevel.L_Text([NSString stringWithFormat:@"当前：%ld 级",[dictData[@"ul_level"] integerValue]]);
    
    _goLevelHint.L_Text(@"继续升级");
