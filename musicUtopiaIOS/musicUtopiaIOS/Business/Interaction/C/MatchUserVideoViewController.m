//
//  MatchUserVideoViewController.m
//  musicUtopiaIOS
//
...
@interface MatchUserVideoViewController ()<UITableViewDelegate,UITableViewDataSource,UITextViewDelegate>
{
    UIView           * _videoBox;
...
@implementation MatchUserVideoViewController
...
-(void)initData:(NSString *)type {
    ...
    NSArray * params = @[@{@"key":@"mvc_mpuid",@"value":@(4)},@{@"key":@"skip",@"value":@(_skip)},@{@"key":@"limit",@"value":@(PAGE_LIMIT)}];
    NSString * url = [G formatRestful:API_MATCH_USER_VIDEO_COMMENT Params:params];
    ...
    [NetWorkTools GET:url params:nil successBlock:^(NSArray *array) {
        ...
        for(NSDictionary * dict in array){
            ...
            MatchUserVideoCommentFrame * frame = [[MatchUserVideoCommentFrame alloc] initWithMatchUserVideoComment:dict];
...
-(void)createVideoBox {
    ...
    //初始化视频
    NSString * playerUrl = @"http://musicalinstrumentutopia.oss-cn-qingdao.aliyuncs.com/userMatchVideo/video2147483647-97473.mp4";
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MatchUserVideoCommentCell * cell   = [[MatchUserVideoCommentCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
    MatchUserVideoCommentFrame * frame = _commentData[indexPath.row];
    cell.matchUserVideoCommentFrame    = frame;
...
-(void)messageSend:(NSString *)msg {
    ...
    //发布动态评论信息
    NSDictionary * params = @{
                            @"mvc_content":_inputTextView.text,
                            @"mvc_mpuid":@(4),
                            @"mvc_uid":@([UserData getUserId])
                            };
    
    NSLog(@"%@",params);
    
    [self startActionLoading:@"评论发布中..."];
    [NetWorkTools POST:API_MATCH_ADD_VIDEO_COMMENT params:params successBlock:^(NSArray *array) {
