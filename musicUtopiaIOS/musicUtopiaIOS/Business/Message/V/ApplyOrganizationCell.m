...
@interface ApplyOrganizationCell()
{
    UIView      * _cellBox;
...
@implementation ApplyOrganizationCell
...
-(void)setDictData:(NSDictionary *)dictData {
    ...
    [MemberInfoData getMemberInfo:dictData[@"oa_apply_username"] MemberEnd:^(NSDictionary *memberInfo) {
    ...
    if([dictData[@"oa_status"] integerValue] == 0){
        _actionLabel.L_Text(@"请求加入该团体");
        _agreedToBtn.hidden  = NO;
        _refusedToBtn.hidden = NO;
    }else if([dictData[@"oa_status"] integerValue] == 1){
        _actionLabel.L_Text(@"您同意了该用户的团体加入请求");
        _agreedToBtn.hidden  = YES;
        _refusedToBtn.hidden = YES;
        
    }else if([dictData[@"oa_status"] integerValue] == 2){
        _actionLabel.L_Text(@"您拒绝了该用户的团体加入请求");
        _agreedToBtn.hidden  = YES;
        _refusedToBtn.hidden = YES;
    }
